...
describe('SearchResults component', () => {
	test('renders the specified number of cards', () => {
		render(<SearchResults pokemons={mockPokemons} onCardClick={() => { }} searchItem="" />);
		const cards = screen.getAllByRole('button');
		expect(cards).toHaveLength(mockPokemons.length);
	});

	test('displays appropriate message if no cards are present', () => {
		render(<SearchResults pokemons={[]} onCardClick={() => { }} searchItem="" />);
		expect(screen.getByText(/No pokemons found/i)).toBeInTheDocument();
	});

	test('renders searchItem specific card', () => {
		render(<SearchResults pokemons={[mockPokemons[0]]} onCardClick={() => { }} searchItem="pikachu" />);
		expect(screen.getByText(/pikachu/i)).toBeInTheDocument();
	});

	test('calls onCardClick when card is clicked', () => {
		const handleCardClick = jest.fn();
		render(<SearchResults pokemons={mockPokemons} onCardClick={handleCardClick} searchItem="" />);
